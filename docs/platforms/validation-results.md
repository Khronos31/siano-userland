...
| Fedora 44 | `0315bd5609422748293bdde63e760c9e02a9cef8` / version `0.1.4` | x86_64 / glibc / SELinux Enforcing。一般ユーザーでPX-S1UDの地上波27chを30秒受信、64,972,800 bytes / 345,600 packets、188-byte remainder 0、sync error 0、保存された検証要約上のAVC拒否0件。Siano専用policy moduleは未導入で、process context、USB node label、raw AVC logは未保存。Stable v0.1.5 archiveのFedora再試験ではない。 | [検証要約と証明範囲](fedora-selinux.md) |
...
| EPSON Endeavor NJ1000 / Debian 12 i386（32-bit / glibc / EHCI） | Stable v0.1.5 source archive（native build SHA-256: `b9bedcf8c44146299dfd10100517eb422360ffc2aad3337c896231275ea404bd`） | Native buildと`make test`が成功し、ELF 32-bit Intel 80386を確認。PX-S1UD 2台のT22/T21同時60秒gateと`/dev/null`向け30分soakは両方lock・exit 0、queue drop 0、process残留0。RSS約24 MiB・FD 8で横ばい、最高53℃、kernel USB/OOM/thermal error 0。別の2 GiB超実ファイル出力ではqueue dropを再現したため、large-file criterionは未合格（Issue #4）。 | 32-bit native buildと非blocking sinkのsoakは合格。slow sink時のdrop通知・終了コードは別課題。 |
| Latitude 5300 / AnduinOS（localhost usbip / VHCI） | Stable v0.1.5 x86_64 archive（SHA-256: `b5528d9f179574b93a2982390d295c595332bf16d1bb60836de775b0c693d931`） | Linux 7.0.0-31-generic、usbip 2.0。network namespace内のloopbackだけで`usbipd`を起動し、PX-S1UD 2台を別VHCI portへattach。VHCI側USB nodeを既存`--fd`で渡した60秒gateと30分soakがpass。T22は3,885,441,120 bytes / 20,667,240 packets、T21は3,885,226,800 bytes / 20,666,100 packets。alignment/sync/TEI/queue drop/fatal error 0、RSS各5,288 KiB・FD各7で一定。終了後にTCP 3240、namespace、VHCI、processの残留0、両デバイスの`smsusb` binding復元を確認。 | 同一ホスト内のusbip/VHCI経路。LAN越し・切断注入は未検証。 |
| Latitude 5300 / AnduinOS（AppArmor enforce） | Stable v0.1.5 x86_64 archive（SHA-256: `b5528d9f179574b93a2982390d295c595332bf16d1bb60836de775b0c693d931`、`siano-ts` SHA-256: `53df5abd54039c41d3df0f130e59d82d5cff16a85b3b3c55f516213501c94818`） | Linux 7.0.0-31-generic、AppArmor 5.0.2。一時的な自作profileでnative USB openのdeny gate、complain 2台30秒、enforce 1台60秒、2台60秒、2台30分を実施。30分はT22 20,667,240 packets、T21 20,666,100 packetsで、両方のalignment/sync/TEI/queue drop/libusb error 0。RSS各5,424 KiB・FD各8で181測定すべて固定。合格case内のAppArmor denial・kernel異常0、profile/process残留0。 | AppArmor profileは配布物に含まれない。ロード済み`smsusb`とのlive handoff境界でkernel warningとallocation failureを観測したため、安全と判定しない。運用上の注意は[AppArmor文書](apparmor.md)を参照。 |
